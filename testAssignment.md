...
data <- read_tsv("/Users/asli/Test Assignment/data/i_youth.tab")
```
...
data$socialmedia <- NA
data$socialmedia[data$i_ypsocweb==1] <- "Yes"
data$socialmedia[data$i_ypsocweb==2] <- "No"
data$socialmedia[data$i_ypsocweb==-9] <- NA
...
``` r
data$gender <- ifelse(data$i_sex_dv==1, "Male", "Female")
table(data$gender)
```
